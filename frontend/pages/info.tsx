      ...
      <div id='info' className='flex h-full mx-auto relative flex-col items-center bg-dark space-y-5 max-w-4xl lg:px-5 pl-12 pr-2 overflow-auto md:overflow-hidden'>
        ...
        <div className='animate-in duration-700 delay-300 fade-in grid grid-cols-2 md:grid-cols-3 gap-2 sm:gap-5 lg:gap-8 w-full max-w-4xl'>
          {infoData.infoImages.map((item, index) => (
            <img key={index} src={urlFor(item)} alt={item.alt} className={`${index === 2 ? 'hidden md:block' : 'block'} w-full h-full max-h-[325px] rounded-[4px] border-2 border-yellow object-cover hover:scale-105 transition-all duration-300 ease-out`} />
          ))}
        </div>
